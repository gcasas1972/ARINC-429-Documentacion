...
/**
*
*  BTICSDB DRIVER EXAMPLE 10  (06/09/2011)
...
DWORD WINAPI EventLogInterruptThread(LPVOID lpParam)
{
	...
	while(pMailbox->fStayAlive)
	{
		...
		else
		{
			...
			printf("\n");
			printf("Msg Addr=%02X   " ,pMailbox->msgfields.data[0]);
			printf("Msg Status=%02X   ",pMailbox->msgfields.data[1]);
			printf("Hit Count=%d",pMailbox->msgfields.hitcount);
