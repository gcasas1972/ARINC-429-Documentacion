...
/**
*
*  LP1553-3 and LC1553-3 DRIVER RTRT EXAMPLE  Version 1.0  (06/29/2004)
...
*  an RT-RT message.  The example also simulates both the
*  receiving RT and the transmitting RT.  RT 1 is the
*  receiving RT, and RT 2 is the transmitting RT.
*
**/
...
void main(void)
{
	...
	MSGADDR RTMsg;						/*RT subaddress message*/
	USHORT  Data[32];					/*32 data words*/

	fprintf(stderr,"\nBCRTRT  Version 1.0  (06/29/2004)");
	fprintf(stderr,"\nCopyright 1997-2004  Ballard Technology, Inc.  Everett, WA, USA.");
	fprintf(stderr,"\nAll rights reserved.");
	fprintf(stderr,"\nGo to www.ballardtech.com or email support@ballardtech.com");
	fprintf(stderr,"\n");
	fprintf(stderr,"\nLP1553-3 and LC1553-3 BCRTRT Example");
	fprintf(stderr,"\n");
...
**/
	...
	errval = L13_BCConfig(BCCFG_DEFAULT,Card);		/*Configure the L13 card as a BC*/
	...
	errval = L13_RTConfig(RTCFG_DEFAULT,1,Card);	/*Configure the L13 card as RT 1*/

	if (errval < 0)
	{
		fprintf(stderr,"\nError:  An error was encountered (%i) while configuring for receiving RT 1.",errval);
		L13_CardClose(Card);
		exit(1);
	}

	errval = L13_RTConfig(RTCFG_DEFAULT,2,Card);	/*Configure the L13 card as RT 2*/

	if (errval < 0)
	{
		fprintf(stderr,"\nError:  An error was encountered (%i) while configuring for transmitting RT 2.",errval);
		L13_CardClose(Card);
		exit(1);
	}
...
/**
*
*  Initialize the transmitting RT with the data to be transmitted.
*
**/

	for (j=0;j<32;++j)									/*Create 32 data words*/
	{
		Data[j] = j;
	}

	RTMsg = L13_RTGetMsg(SUBADDRESS, 2, 1, 5, Card);	/*Get address of subaddress*/

	L13_MsgDataWr(Data, 32, RTMsg, Card);				/*Write 32 data words to subaddress*/
														/*Command word will determine how many datawords to transmit*/
