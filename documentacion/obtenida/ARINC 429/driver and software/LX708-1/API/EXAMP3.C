...
/**
*
*  LP708-1 and LC708-1 DRIVER EXAMPLE 3  Version 1.1  (01/19/2001)
...
**/
	...
	errval = L71_XmtConfigEx(framefreq,blocksize,framecount,Card);   /*Configure the L71 card*/
...
**/
	...
	for (j=0;j<framecount;++j)             /*Loop through each of the frames*/
	{
		if (fread(Data,1,sizeof(Data),fh) == 0)  /*If at end of file*/
		{
			...
			L71_XmtWr(Data,j,Card);	   /*Write the data for frame*/
		}
		else
		{
			L71_XmtWr(Data,j,Card);        /*Write the data for frame*/
...
**/
	...
	while (!_kbhit())
	{
		while(L71_IntStatus(Card) != STAT_EMPTY)   /*Check if the interrupt log list is not empty*/
		{
			L71_IntRd(NULL,&framenum,Card);        /*Read an entry from the interrupt log list*/
