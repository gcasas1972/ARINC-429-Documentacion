...
*  ALL RIGHTS RESERVED
*
*  NAME:   PM11.C --  PM1553-1 WIN32 Driver Source File.
*
**/
...
*
*   BOOL    PM11_CardOpen()         -- Returns a handle to a PM1553-1
*   BOOL    PM11_CardClose()        -- Close a PM1553-1
*   BOOL    PM11_IntInstall()       -- Install an event to handle interrupts
*   BOOL    PM11_IntUninstall()     -- Uninstall event
*   BOOL    PM11_IntReset()         -- Clear pending interrupt
*
*  WIN32 Only Utility functions
*
*   HANDLE  PM11_UtilFindCard()     -- Finds the card HANDLE associated with the port
*   BOOL    PM11_UtilPushCard()     -- Adds a card to the table, using its port as the key
*   BOOL    PM11_UtilPopCard()      -- Removes a card from the table, using its port as the key
*
*  Utility functions
*
...
*   PM11_MAXCARDS can be changed to any appropriate value.
*
*/

#define PM11_MAXCARDS 5

BOOL bInitialized=FALSE;
ULONG ulCards[PM11_MAXCARDS][2];
...
*	Warnings: Do not use this function outside of PM11_CardOpen/Close.
*
**/

HANDLE PM11_UtilFindCard(USHORT port)
{
	INT j;

	if (FALSE == bInitialized)
	{
		return(PM11_INVALID_HANDLE);
	}

	for (j=0;j<PM11_MAXCARDS;++j)
	{
...
BOOL PM11_UtilPushCard(USHORT port,HANDLE card)
{
	INT j;
	...
	if (FALSE == bInitialized)
	{
		memset(ulCards, (ULONG)PM11_INVALID_HANDLE, PM11_MAXCARDS*2*sizeof(ulCards[0][0]));
		bInitialized = TRUE;
	}
	...
	for (j=0;j<PM11_MAXCARDS;++j)
	{
		if (ulCards[j][0] == (ULONG)PM11_INVALID_HANDLE)
		{
...
BOOL PM11_UtilPopCard(USHORT port)
{
	INT i;

	if (FALSE == bInitialized) return(FALSE);

	for (i = 0; i < PM11_MAXCARDS; ++i)
	{
		if (ulCards[i][0] == port)
		{
			ulCards[i][0] = (ULONG)PM11_INVALID_HANDLE;
			ulCards[i][1] = (ULONG)PM11_INVALID_HANDLE;
			return(TRUE);
		}
	}
...
BOOL PM11_IntInstall(HANDLE hEvent,USHORT port)
{
	...
	PM11_IntEnable(PCIEN,port);

	//
	//	Register event object with kernel driver.
	//

	result = BTIKRNL_InterruptEnable(hEvent,PM11_UtilFindCard(port));
...
BOOL PM11_IntUninstall(USHORT port)
{
	...
	result = BTIKRNL_InterruptDisable(0,PM11_UtilFindCard(port));
	if (!result) return(FALSE);
...
BOOL PM11_IntReset(USHORT port)
{
	...
	result = BTIKRNL_InterruptClear(PM11_UtilFindCard(port));
	if (!result) return(FALSE);
...
VOID PM11_Outp(UCHAR data, USHORT port)
{
	...
	BTIKRNL_PortWrB(data, (port & 0xff0), 0, PM11_UtilFindCard(port));
...
UCHAR PM11_Inp(USHORT port)
{
	...
	BTIKRNL_PortRdB(&val, (port & 0x00f), 0, PM11_UtilFindCard(port));
#else
#  ifdef _MSC_VER             // if using MS Visual C, use the MS name
...
#  ifdef _WIN32         // if compiling for NT
	// (port & 0x00f) gives the LSB, which should be the offset.
	BTIKRNL_PortWrW(data, (port & 0x00f), 0, PM11_UtilFindCard(port));
#  else
#    ifdef _MSC_VER      // if using MS Visual C, use the MS name
...
#  ifdef _WIN32         // if compiling for NT
	USHORT sVal = 0;
	// (port & 0x00f) gives the LSB, which should be the offset.
	BTIKRNL_PortRdW(&sVal, (port & 0x00f), 0, PM11_UtilFindCard(port));
	return sVal;
#  else
#    ifdef _MSC_VER      // if using MS Visual C, use the MS name
...
*
*	PM11_CardOpen()
*	Obtains access to a PM1553-1.
*	Arguments:
*		port: PM1553-1 port setting
*	Returns:  Handle to a card.
*	Warnings: Be sure to have a companion PM11_CardClose call for each
*             PM11_CardOpen call.
...
HANDLE PM11_CardOpen(LPCTSTR name,USHORT port)
{
	...
	HANDLE hCard = PM11_UtilFindCard(port);
	...
	if (hCard)
	{
		if (FALSE == PM11_UtilPushCard(port, hCard))
		{
			return(PM11_ERRMEM_HANDLE);
		}
		else
		{
			return(hCard);
...
*
*	PM11_CardClose()
*	Releases access to the PM1553-1 card previously gained by PM11_CardOpen.
*	Arguments:
*		port: PM1553-1 port setting
...
BOOL PM11_CardClose(USHORT port)
{
	...
	if (FALSE == PM11_UtilPopCard(port))
	{
		return(FALSE);
	}
...
INT PM11_ROMWr(UCHAR data, UINT addr, USHORT port)
{
	...
	PM11_RegWr(0x00AA, CREG_DATA, port);
	PM11_RegWr(0x02AA, CREG_ADDR, port);
	PM11_RegWr(0x0055, CREG_DATA, port);
	PM11_RegWr(0x0555, CREG_ADDR, port);
	PM11_RegWr(0x00A0, CREG_DATA, port);
...
INT PM11_ROMWipe(USHORT port)
{
	...
	PM11_RegWr(0x0055, CREG_DATA, port);
	PM11_RegWr(0x0555, CREG_ADDR, port);
	PM11_RegWr(0x0080, CREG_DATA, port);
	PM11_RegWr(0x0555, CREG_ADDR, port);
	PM11_RegWr(0x00AA, CREG_DATA, port);
	PM11_RegWr(0x02AA, CREG_ADDR, port);
	PM11_RegWr(0x0055, CREG_DATA, port);
	PM11_RegWr(0x0555, CREG_ADDR, port);
	PM11_RegWr(0x0010, CREG_DATA, port);
