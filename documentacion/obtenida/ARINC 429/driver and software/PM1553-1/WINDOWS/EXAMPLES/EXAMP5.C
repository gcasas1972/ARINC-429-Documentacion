/**
*
*  PM1553-1 EXAMPLE 5  Version 1.0  (04/04/2006)
...
*  ALL RIGHTS RESERVED
*
*  NAME:   EXAMP5.C -- "Example of BC Mode with interrupts."
*
**/
...
*
*  Transmit Message C only if SRQ bit is set in status word of Message B and
*  issue interrupt.
*
**/
...
struct Mailbox						//Variables used by the thread
{
	...
	INT       hits;					//Count of interrupt hits
...
DWORD WINAPI InterruptServiceThread(LPVOID lpParam)
{
	...
	while(pMailbox->fStayAlive)
	{
		if (WaitForSingleObject(pMailbox->hEvent,200) != WAIT_TIMEOUT)
		{
			...
			PM11_SRQClr(MSGINT,pMailbox->port);			//Clear MSG SRQ

			PM11_IntReset(pMailbox->port);				//Reset interrupt
...
VOID main(VOID)
{
	...
	if (fWindowsNT40)
	{
		ThreadMailbox.hEvent = OpenEvent(
			...
			FALSE,						//Inherit flag
			"PM1553Interrupt");			//Pointer to event-object name
	...
	PM11_IntEnable(MSGIEN,port);

	//
	//	Install the interrupt handler.
	//

	result = PM11_IntInstall(ThreadMailbox.hEvent,port);
	...
	{
		fprintf(stderr,"\rInterrupt hits=%d",ThreadMailbox.hits);
